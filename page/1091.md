...
为了访问结构的成员，我们使用**成员访问运算符（.）**。成员访问运算符是结构变量名称和我们要访问的结构成员之间的一个句号。可以使用 **struct** 关键字来定义结构类型的变量。下面的实例演示了结构的用法：
...
int main( )
{
   struct page Book1;        /* 声明 Book1，类型为 page */
   struct page Book2;        /* 声明 Book2，类型为 page */
...
/* 函数声明 */
void printBook( struct page book );
int main( )
{
   struct page Book1;        /* 声明 Book1，类型为 page */
   struct page Book2;        /* 声明 Book2，类型为 page */
